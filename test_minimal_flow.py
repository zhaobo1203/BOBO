...
测试流程：
脚本启动 → 模块1启动 → 获取历史消息 → 模块3启动 → 模块2启动 → 模块之间数据正常获取并显示
...
模块间依赖关系：
- 模块1独立运行，产生messages.db
- 模块2独立运行，产生a_stock.db
- 模块3依赖模块1和模块2的输出
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(PROJECT_ROOT / "logs" / "minimal_flow_test.log", encoding='utf-8')
    ]
)
...
class MinimalFlowTest:
    ...
    def print_header(self):
        ...
        print("=" * 70)
        print("  最小化流程测试")
        print("  流程: 模块1启动 → 历史消息 → 模块3启动 → 模块2启动 → 数据验证")
        print("=" * 70)
        print()
    ...
    def step0_setup(self):
        """环境准备"""
        self.print_header()
        self.print_step("步骤0: 环境检查", "doing")
    ...
    def step1_start_module1(self):
        """启动模块1 - 微信消息监听"""
        print()
        self.print_step("步骤1: 模块1启动", "info", "微信消息监听")
        ...
        try:
            ...
            if not self.monitor_instance.step3_get_key():
                ...
                return False
            self.print_step("1.3 密钥获取", "done", "成功")
    ...
    def step1_5_get_history_messages(self) -> bool:
        """获取历史消息并保存到messages.db"""
        print()
        self.print_step("步骤1.5: 获取历史消息", "info")
        
        try:
            # 使用SimpleMonitor的方法获取群列表
    ...
    def step2_start_module3(self) -> bool:
        """启动模块3 - 股票分析服务"""
        print()
        self.print_step("步骤2: 模块3启动", "info", "股票分析服务")
    ...
    def step3_start_module2(self) -> bool:
        """启动模块2 - A股数据更新"""
        print()
        self.print_step("步骤3: 模块2启动", "info", "A股数据更新")
    ...
    def step4_trigger_match(self) -> bool:
        """触发模块3全量匹配"""
        print()
        self.print_step("步骤4: 触发匹配", "info")
    ...
    def step5_verify_data(self) -> bool:
        """验证各模块数据"""
        print()
        self.print_step("步骤5: 数据验证", "info")
    ...
    def run(self):
        ...
        try:
            ...
            # 步骤1.5: 获取历史消息
            if not self.step1_5_get_history_messages():
                return False
            
            # 步骤2: 模块3启动（先启动模块3，因为模块3会自动初始化匹配）
            if not self.step2_start_module3():
                return False
            ...
            # 步骤4: 触发全量匹配
            if not self.step4_trigger_match():
                return False
            
            # 步骤5: 数据验证
            if not self.step5_verify_data():
                self.print_step("数据验证", "fail", "数据不完整")
                return False
            
            # 步骤6: 显示看板
            self.step6_show_dashboard()
